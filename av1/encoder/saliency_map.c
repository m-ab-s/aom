...
// (r+g+b)/3.
static void av1_get_color_intensity(YV12_BUFFER_CONFIG *src, double *Cr,
                                    double *Cg, double *Cb, double *Intensity) {
...
static INLINE double convolve_map(const double *filter, const double *img,
                                  const int size) {
  double result = 0;
...
static INLINE void upscale_map(double *input, int in_level, double *output,
                               int out_level, int height[9], int width[9]) {
...
// where delta \in {3, 4}.
static void Center_surround_diff(double *input[9],
...
// differences.
static void Gaussian_Pyramid(saliency_feature_map *dst[6], double *src,
...
static INLINE void Filter2D(double *input, double *output,
...
static void Get_Feature_Map_Orientation(saliency_feature_map *dst[24],
  ...
  for (int i = 2; i < 9; i++) {
    ...
    tempGaborOutput135[i] =
        (double *)malloc(cur_height * cur_width * sizeof(double));
    Filter2D(GaussianMap[i], tempGaborOutput0[i], GaborFilter0, cur_width,
             cur_height);
    Filter2D(GaussianMap[i], tempGaborOutput45[i], GaborFilter45, cur_width,
             cur_height);
    Filter2D(GaussianMap[i], tempGaborOutput90[i], GaborFilter90, cur_width,
             cur_height);
    Filter2D(GaussianMap[i], tempGaborOutput135[i], GaborFilter135, cur_width,
             cur_height);
  }
...
static INLINE void FindMinMax(saliency_feature_map *input, double *maxx,
                              double *minn) {
...
static void normalizeFM(saliency_feature_map *input[6],
