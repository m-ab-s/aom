...
static void wht_fwd_txfm(int16_t *src_diff, int bw, tran_low_t *coeff,
                         TX_SIZE tx_size, int bit_depth, int is_hbd) {
...
static uint32_t motion_estimation(AV1_COMP *cpi, MACROBLOCK *x,
  ...
  xd->mi[0]->pb_mv_precision = max_mv_precision;
...
static void mode_estimation(AV1_COMP *cpi, MACROBLOCK *x, MACROBLOCKD *xd,
                            ...
                            const YV12_BUFFER_CONFIG *src_ref_frame[],
                            uint8_t *predictor, int base_rdmult,
                            int64_t *recon_error, int64_t *sse,
  ...
  for (mode = DC_PRED; mode <= PAETH_PRED; ++mode) {
    ...
    av1_predict_intra_block(
        ...
        dst_buffer, dst_buffer_stride, dst, dst_stride, 0, 0, 0);
...
static void mc_flow_dispenser(AV1_COMP *cpi, int frame_idx, int pframe_qindex) {
  const GF_GROUP *gf_group = &cpi->gf_group;
