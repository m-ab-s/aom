...
#endif  // CONFIG_NEW_REF_SIGNALING
#if CONFIG_NEW_REF_SIGNALING
  const uint8_t ref_frame_type = av1_ref_frame_type_nrs(ref_frames_nrs);
#else
...
#if CONFIG_NEW_REF_SIGNALING
      const MV_REFERENCE_FRAME_NRS ref_frame_nrs0 =
  ...
  if (is_comp_pred) {
    ...
    if (this_mode == NEW_NEWMV) {
      ...
      if (cpi->sf.inter_sf.comp_inter_joint_search_thresh <= bsize ||
        ...
        *rate_mv = 0;
        for (int i = 0; i < 2; ++i) {
          ...
          *rate_mv +=
              av1_mv_bit_cost(&cur_mv[i].as_mv, &ref_mv.as_mv, max_mv_precision,
      ...
      if (cpi->sf.inter_sf.comp_inter_joint_search_thresh <= bsize ||
        ...
        *rate_mv =
            av1_mv_bit_cost(&cur_mv[1].as_mv, &ref_mv.as_mv, max_mv_precision,
                            &x->mv_costs, MV_COST_WEIGHT);
      }
    } else {
#if CONFIG_NEW_INTER_MODES
      ...
      if (cpi->sf.inter_sf.comp_inter_joint_search_thresh <= bsize ||
        ...
        *rate_mv =
            av1_mv_bit_cost(&cur_mv[0].as_mv, &ref_mv.as_mv, max_mv_precision,
                            &x->mv_costs, MV_COST_WEIGHT);
      }
    }
  } else {
...
static INLINE void refine_mv_for_newmv(const AV1_COMP *const cpi,
                                       MACROBLOCK *const x, BLOCK_SIZE bsize,
                                       const int *pts0, const int *pts_inref0,
                                       int total_samples, int *tmp_rate_mv,
                                       int *tmp_rate2, int rate2_nocoeff,
                                       int rate_mv0) {
  ...
  if (mv0.as_int != mbmi->mv[0].as_int) {
    ...
    *tmp_rate_mv =
        av1_mv_bit_cost(&mbmi->mv[0].as_mv, &ref_mv.as_mv, max_mv_precision,
                        &x->mv_costs, MV_COST_WEIGHT);
  ...
  for (int mode_index = mode_index_start; mode_index <= mode_index_end;
    ...
    *mbmi = base_mbmi;
    ...
    if (mbmi->motion_mode == SIMPLE_TRANSLATION && !is_interintra_mode) {
      ...
      const int tmp_rate2_0 = tmp_rate2;
      const int tmp_rate_mv0 = tmp_rate_mv;
      // keep track of best rotation degree
      int64_t rdcost = INT64_MAX;
      int best_rot = 0;
      ...
      for (int rot = -ROTATION_RANGE; rot <= ROTATION_RANGE;
        ...
        if (!av1_find_projection(mbmi->num_proj_ref, pts, pts_inref, bsize,
          ...
          if (have_newmv_in_inter_mode(this_mode)) {
            // Refine MV for NEWMV mode
            refine_mv_for_newmv(cpi, x, bsize, pts0, pts_inref0, total_samples,
                                &tmp_rate_mv, &tmp_rate2, rate2_nocoeff,
                                rate_mv0);
          }
...
#if CONFIG_NEW_REF_SIGNALING
  const uint8_t ref_frame_type = av1_ref_frame_type_nrs(ref_frame);
#else
  ...
  for (int idx = 0; idx < range; ++idx) {
    ...
    switch (idx) {
      case 0:
        ...
        break;
      case 1:
        cost += x->mode_costs.drl1_mode_cost[drl_ctx][mbmi->ref_mv_idx != idx];
        break;
      default:
        cost += x->mode_costs.drl2_mode_cost[drl_ctx][mbmi->ref_mv_idx != idx];
        break;
    }
...
static bool ref_mv_idx_early_breakout(
...
#if CONFIG_NEW_REF_SIGNALING
  const int8_t ref_frame_type = av1_ref_frame_type_nrs(mbmi->ref_frame_nrs);
#else
  ...
  if (sf->inter_sf.reduce_inter_modes && ref_mv_idx > 0) {
    ...
    if (mbmi->ref_frame[0] == LAST2_FRAME ||
        ...
        mbmi->ref_frame[1] == LAST2_FRAME || mbmi->ref_frame[1] == LAST3_FRAME)
#endif  // CONFIG_NEW_REF_SIGNALING
    {
#if CONFIG_NEW_INTER_MODES
...
static int skip_repeated_newmv(
    ...
    const MvSubpelPrecision max_mv_precision, MB_MODE_INFO *best_mbmi,
    ...
    HandleInterModeArgs *args, int drl_cost,
#if CONFIG_NEW_REF_SIGNALING
    const MV_REFERENCE_FRAME_NRS *refs,
#else
    const MV_REFERENCE_FRAME *refs,
...
static int64_t rd_pick_intrabc_mode_sb(const AV1_COMP *cpi, MACROBLOCK *x,
  ...
  av1_make_default_fullpel_ms_params(
      &fullms_params, cpi, x, bsize, &dv_ref.as_mv,
      cm->features.fr_mv_precision, lookahead_search_sites,
...
static AOM_INLINE void disable_reference_nrs(
  ...
  for (MV_REFERENCE_FRAME_NRS ref2 = INVALID_IDX; ref2 < REF_FRAMES_NRS;
       ++ref2) {
    ref_combo[ref][ref2 + 1] = true;
  }
}

static const MV_REFERENCE_FRAME_NRS reduced_ref_combos_nrs[][2] = {
  { 0, INVALID_IDX },
  { 1, INVALID_IDX },
  { 2, INVALID_IDX },
  { 3, INVALID_IDX },
...
static const MV_REFERENCE_FRAME_NRS real_time_ref_combos_nrs[][2] = {
  { 0, INVALID_IDX },
  { 1, INVALID_IDX },
  { 2, INVALID_IDX },
  { INTRA_FRAME_NRS, INVALID_IDX },
...
static void set_mask_combo_bit_nrs(
    bool ref_combo[REF_FRAMES_NRS][REF_FRAMES_NRS + 1],
    const MV_REFERENCE_FRAME_NRS *refs, bool val) {
  const MV_REFERENCE_FRAME_NRS r0 =
      (refs[0] == INTRA_FRAME_NRS ? INTER_REFS_PER_FRAME_NRS : refs[0]);
  const MV_REFERENCE_FRAME_NRS r1 =
      (refs[1] == INTRA_FRAME_NRS ? INTER_REFS_PER_FRAME_NRS : refs[1]);
...
static AOM_INLINE void default_skip_mask_nrs(mode_skip_mask_t *mask,
  ...
  if (ref_set == REF_SET_FULL) {
    ...
    for (int i = 0; i < num_ref_combos; ++i) {
      const MV_REFERENCE_FRAME_NRS *const this_combo = ref_set_combos[i];
...
static AOM_INLINE void init_mode_skip_mask_nrs(mode_skip_mask_t *mask,
  ...
  mask->pred_modes[INTER_REFS_PER_FRAME_NRS] |=
...
#if CONFIG_NEW_REF_SIGNALING
      for (int ref_frame = 0; ref_frame <= INTER_REFS_PER_FRAME_NRS;
           ++ref_frame) {
#else
...
static int inter_mode_compatible_skip_nrs(
    ...
    PREDICTION_MODE curr_mode, const MV_REFERENCE_FRAME_NRS *ref_frames) {
  const int comp_pred =
  ...
  if (ref_frames[0] != INTRA_FRAME_NRS && ref_frames[0] != INVALID_IDX &&
      ref_frames[1] == INTRA_FRAME_NRS) {
...
#if CONFIG_NEW_REF_SIGNALING
  const MV_REFERENCE_FRAME_NRS *cached_frame = cached_mi->ref_frame_nrs;
...
#if CONFIG_NEW_REF_SIGNALING
    const int mode_is_single =
        ref_frame[1] == INTRA_FRAME_NRS || ref_frame[1] == INVALID_IDX;
#else
...
static int compound_skip_by_single_states(
...
#if CONFIG_NEW_REF_SIGNALING
    const MV_REFERENCE_FRAME_NRS ref_frame,
    const MV_REFERENCE_FRAME_NRS second_ref_frame,
#else
    const MV_REFERENCE_FRAME ref_frame,
    const MV_REFERENCE_FRAME second_ref_frame,
#endif  // CONFIG_NEW_REF_SIGNALING
    const MACROBLOCK *x) {
#if CONFIG_NEW_REF_SIGNALING
  const MV_REFERENCE_FRAME_NRS refs[2] = { ref_frame, second_ref_frame };
#else
...
#endif  // CONFIG_NEW_INTER_MODES
#if CONFIG_NEW_REF_SIGNALING
    const MV_REFERENCE_FRAME_NRS single_refs[2] = { refs[i], INVALID_IDX };
#else
...
static INLINE void match_ref_frame(const MB_MODE_INFO *const mbmi,
                                   const MV_REFERENCE_FRAME_NRS *ref_frames,
...
static INLINE void update_best_single_mode(
    ...
    int64_t this_rd) {
#if CONFIG_NEW_REF_SIGNALING
  const MV_REFERENCE_FRAME_NRS ref_frame_nrs = COMPACT_INDEX0_NRS(ref_frame);
...
static AOM_INLINE void find_top_ref(int64_t ref_frame_rd[REF_FRAMES]) {
  ...
  // and store +10% of it in the INTRA_FRAME_INDEX_NRS-th element in
  // ref_frame_rd.
...
static INLINE bool in_single_ref_cutoff(int64_t ref_frame_rd[REF_FRAMES],
                                        MV_REFERENCE_FRAME_NRS frame1,
                                        MV_REFERENCE_FRAME_NRS frame2) {
  assert(frame2 != INVALID_IDX && frame2 != INTRA_FRAME_NRS);
...
static int skip_inter_mode(AV1_COMP *cpi, MACROBLOCK *x, const BLOCK_SIZE bsize,
  ...
  MB_MODE_INFO *const mbmi = xd->mi[0];
#if CONFIG_NEW_REF_SIGNALING
  const MV_REFERENCE_FRAME_NRS ref_frame_nrs = ref_frames_nrs[0];
  (void)ref_frame_nrs;
  const MV_REFERENCE_FRAME_NRS second_ref_frame_nrs = ref_frames_nrs[1];
...
#if CONFIG_NEW_REF_SIGNALING
    const MV_REFERENCE_FRAME_NRS refs[2] = { mbmi->ref_frame_nrs[0],
                                             mbmi->ref_frame_nrs[1] };
#else
    const MV_REFERENCE_FRAME refs[2] = { mbmi->ref_frame[0],
                                         mbmi->ref_frame[1] };
...
void av1_rd_pick_inter_mode_sb(struct AV1_COMP *cpi,
  ...
  if (cpi->sf.inter_sf.prune_inter_modes_based_on_tpl) {
    ...
    for (MV_REFERENCE_FRAME_NRS frame = 0; frame < INTER_REFS_PER_FRAME_NRS;
         frame++) {
      const MV_REFERENCE_FRAME_NRS refs[2] = { frame, INVALID_IDX };
