...
void av1_fill_mode_rates(AV1_COMMON *const cm, ModeCosts *mode_costs,
  ...
  for (i = 0; i < KF_MODE_CONTEXTS; ++i) {
    for (j = 0; j < KF_MODE_CONTEXTS; ++j) {
      av1_cost_tokens_from_cdf(mode_costs->kf_is_dr_mode_cost[i][j],
  ...
  for (i = 0; i < KF_MODE_CONTEXTS; ++i) {
    for (j = 0; j < KF_MODE_CONTEXTS; ++j) {
      av1_cost_tokens_from_cdf(mode_costs->kf_dr_mode_cost[i][j],
  ...
  for (i = 0; i < KF_MODE_CONTEXTS; ++i) {
    for (j = 0; j < KF_MODE_CONTEXTS; ++j) {
      av1_cost_tokens_from_cdf(mode_costs->kf_none_dr_mode_cost[i][j],
  ...
  if (!frame_is_intra_only(cm)) {
    ...
    for (i = 0; i < DRL_MODE_CONTEXTS; ++i) {
      av1_cost_tokens_from_cdf(mode_costs->drl0_mode_cost[i], fc->drl0_cdf[i],
                               NULL);
      av1_cost_tokens_from_cdf(mode_costs->drl1_mode_cost[i], fc->drl1_cdf[i],
                               NULL);
      av1_cost_tokens_from_cdf(mode_costs->drl2_mode_cost[i], fc->drl2_cdf[i],
                               NULL);
    }
#else
    ...
    for (i = 0; i < BLOCK_SIZES_ALL; i++) {
      av1_cost_tokens_from_cdf(mode_costs->warp_rotation_flag_cost[i],
                               fc->warp_rotation_flag_cdf[i], NULL);
    }
    av1_cost_tokens_from_cdf(mode_costs->warp_rotation_degree_cost,
                             fc->warp_rotation_degree_cdf, NULL);

    for (i = 0; i < BLOCK_SIZES_ALL; i++) {
      av1_cost_tokens_from_cdf(mode_costs->globalmv_rotation_flag_cost[i],
                               fc->globalmv_rotation_flag_cdf[i], NULL);
    }
    av1_cost_tokens_from_cdf(mode_costs->globalmv_rotation_degree_cost,
                             fc->globalmv_rotation_degree_cdf, NULL);

    for (i = 0; i < BLOCK_SIZES_ALL; i++) {
      av1_cost_tokens_from_cdf(mode_costs->translation_rotation_flag_cost[i],
                               fc->translation_rotation_flag_cdf[i], NULL);
    }
    av1_cost_tokens_from_cdf(mode_costs->translation_rotation_degree_cost,
                             fc->translation_rotation_degree_cdf, NULL);
...
#if CONFIG_NEW_REF_SIGNALING
  for (PREDICTION_MODE mode = 0; mode < top_mode; ++mode) {
#else
