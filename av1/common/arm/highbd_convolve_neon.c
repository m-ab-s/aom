...
static INLINE uint16x4_t highbd_convolve6_4_x(const int16x4_t s[6],
  ...
  sum = vmlal_lane_s16(sum, s[0], x_filter_0_3, 1);
...
static INLINE void highbd_convolve_x_sr_6tap_neon(
  ...
  if (w == 4) {
    ...
    do {
      ...
      uint16x4_t d0 = highbd_convolve6_4_x(s0, x_filter, offset);
      uint16x4_t d1 = highbd_convolve6_4_x(s1, x_filter, offset);
      uint16x4_t d2 = highbd_convolve6_4_x(s2, x_filter, offset);
      uint16x4_t d3 = highbd_convolve6_4_x(s3, x_filter, offset);
...
static INLINE uint16x4_t highbd_convolve6_4_2d_h(const int16x4_t s[6],
                                                 const int16x8_t x_filter,
...
static INLINE void highbd_convolve_2d_sr_horiz_6tap_neon(
  ...
  if (w == 4) {
    ...
    do {
      ...
      uint16x4_t d1 =
          highbd_convolve6_4_2d_h(s1, x_filter, shift_s32, offset_s32);
      uint16x4_t d2 =
          highbd_convolve6_4_2d_h(s2, x_filter, shift_s32, offset_s32);
      uint16x4_t d3 =
          highbd_convolve6_4_2d_h(s3, x_filter, shift_s32, offset_s32);
...
static INLINE void highbd_convolve_2d_sr_horiz_8tap_neon(
    const uint16_t *src_ptr, int src_stride, uint16_t *dst_ptr, int dst_stride,
    int w, int h, const int16_t *x_filter_ptr, ConvolveParams *conv_params,
    const int offset) {
  const int16x8_t x_filter = vld1q_s16(x_filter_ptr);
  ...
  if (w == 4) {
    ...
    do {
      ...
      uint16x4_t d2 =
          highbd_convolve8_4_2d_h(s2, x_filter, shift_s32, offset_s32);
      uint16x4_t d3 =
          highbd_convolve8_4_2d_h(s3, x_filter, shift_s32, offset_s32);
...
void av1_highbd_convolve_2d_sr_neon(const uint16_t *src, int src_stride,
  ...
  if (x_filter_taps <= 6) {
    ...
    highbd_convolve_2d_sr_horiz_8tap_neon(src_ptr, src_stride, im_block,
                                          im_stride, w, im_h, x_filter_ptr,
                                          conv_params, x_offset_initial);
  }
