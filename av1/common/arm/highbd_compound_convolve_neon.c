...
static INLINE uint16x4_t highbd_convolve6_4_x(const int16x4_t s[6],
  ...
  int32x4_t sum = vmlal_lane_s16(offset, s[0], x_filter_0_3, 1);
  sum = vmlal_lane_s16(sum, s[1], x_filter_0_3, 2);
  sum = vmlal_lane_s16(sum, s[2], x_filter_0_3, 3);
  sum = vmlal_lane_s16(sum, s[3], x_filter_4_7, 0);
  sum = vmlal_lane_s16(sum, s[4], x_filter_4_7, 1);
  sum = vmlal_lane_s16(sum, s[5], x_filter_4_7, 2);
...
static INLINE void highbd_dist_wtd_convolve_x_6tap_neon(
  ...
  if (w == 4) {
    ...
    do {
      ...
      uint16x4_t d0 = highbd_convolve6_4_x(s0, x_filter, shift, offset_vec);
      uint16x4_t d1 = highbd_convolve6_4_x(s1, x_filter, shift, offset_vec);
      uint16x4_t d2 = highbd_convolve6_4_x(s2, x_filter, shift, offset_vec);
      uint16x4_t d3 = highbd_convolve6_4_x(s3, x_filter, shift, offset_vec);
    ...
    do {
      ...
      do {
        ...
        uint16x8_t d0 = highbd_convolve6_8_x(s0, x_filter, shift, offset_vec);
        uint16x8_t d1 = highbd_convolve6_8_x(s1, x_filter, shift, offset_vec);
        uint16x8_t d2 = highbd_convolve6_8_x(s2, x_filter, shift, offset_vec);
        uint16x8_t d3 = highbd_convolve6_8_x(s3, x_filter, shift, offset_vec);
